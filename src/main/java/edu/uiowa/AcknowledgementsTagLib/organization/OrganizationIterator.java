...
public class OrganizationIterator extends AcknowledgementsTagLibBodyTagSupport {
    int ID = 0;
	...
	public static String organizationCount() throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from pubmed_central_ack_stanford.organization"
						);
	...
	public static Boolean organizationExists (String ID) throws JspTagException {
		...
		OrganizationIterator theIterator = new OrganizationIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from pubmed_central_ack_stanford.organization where 1=1"
						+ " and id = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT pubmed_central_ack_stanford.organization.id from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("pubmed_central_ack_stanford.organization");
