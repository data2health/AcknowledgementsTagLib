...
public class AwardDeleter extends AcknowledgementsTagLibBodyTagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from pubmed_central_ack_stanford.award where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? "));
            ...
            log.error("JDBC error generating Award deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating Award deleter");
        } finally {
            freeConnection();
        }
