...
public class PersonDeleter extends AcknowledgementsTagLibBodyTagSupport {
    int ID = 0;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from pubmed_central_ack_stanford.person where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? "));
            ...
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating Person deleter");
        } finally {
            freeConnection();
        }
