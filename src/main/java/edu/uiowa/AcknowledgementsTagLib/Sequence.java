...
public class Sequence extends AcknowledgementsTagLibTagSupport {
	...
	static public int generateID() {
		...
		try {
			...
			switch (theDriver) {
				case POSTGRESQL:
					PreparedStatement stat = conn.prepareStatement("SELECT nextval ('pubmed_central_ack_stanford.seqnum')");
