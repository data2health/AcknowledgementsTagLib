...
public class ProjectDeleter extends AcknowledgementsTagLibBodyTagSupport {
    int ID = 0;
    String project = null;
	Vector<AcknowledgementsTagLibTagSupport> parentEntities = new Vector<AcknowledgementsTagLibTagSupport>();

	private static final Log log = LogFactory.getLog(ProjectDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from pubmed_central_ack_stanford.project where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? "));
